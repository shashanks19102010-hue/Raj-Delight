...
const categoryImages: Record<string, string> = {
  ...
  'South Indian': foodImages.dosa, 'Street Smart': foodImages.pav, Dessert: foodImages.dessert,
...
export function RajDelightExperienceV10() {
  ...
  return <main className={styles.site}>
    ...
    <section id="menu" className={`${styles.section} ${styles.menuSection}`} data-reveal><div className={styles.wrap}><div className={styles.menuHead}><div><p className={styles.eyebrow}>03 · THE COMPLETE MENU</p><h2>Everything on the <em>table.</em></h2><p>{menuItemCount} public menu items across {menuCategories.length} categories.</p></div><div className={styles.menuStamp}><span className={styles.menuStampBadge}>₹</span><div><small>MENU BOOK · 2024</small><strong>{menuItemCount} dishes · {menuCategories.length} categories</strong><p>Prices transcribed from the supplied menu.</p></div><Icon name="spark"/></div></div><div className={styles.controls}><label className={styles.search}><Icon name="search"/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search dishes or categories" aria-label="Search the menu"/></label><div className={styles.chips}><button className={category === 'All' ? styles.activeChip : ''} onClick={() => setCategory('All')}>All <span>{menuItemCount}</span></button>{visibleCategories.slice(0, categoriesOpen || query.trim() ? visibleCategories.length : 12).map((entry) => <button key={entry.name} className={category === entry.name ? styles.activeChip : ''} onClick={() => setCategory(entry.name)}>{entry.name} <span>{entry.items.length}</span></button>)}{!query.trim() && visibleCategories.length > 12 && <button className={styles.moreChip} onClick={() => setCategoriesOpen((v) => !v)}>{categoriesOpen ? 'Show less' : `+${visibleCategories.length - 12} more`}</button>}</div></div><div className={styles.menuSummaryRow}><div className={styles.menuSummary}>{results.length} dishes shown</div><div className={styles.priceLegend}><span>₹</span> Prices from the supplied Raj Delight Menu 2024</div></div>{results.length === 0 ? <div className={styles.empty}><strong>No dishes found.</strong><p>Try another dish name or category.</p><button onClick={() => { setCategory('All'); setQuery(''); }}>Reset search</button></div> : <div className={styles.menuGrid}>{menuCategories.map((entry, index) => { const items = results.filter((item) => item.category === entry.name); if (!items.length) return null; return <article className={styles.chapter} key={entry.name}><button className={styles.chapterHead} onClick={() => setCategory(entry.name)}><span>{String(index + 1).padStart(2, '0')}</span><strong>{entry.name}</strong><b>{items.length}</b></button><div className={styles.items}>{items.map((item, itemIndex) => { const price = getMenuPrice(entry.name, item.name); const label = price === null ? item.name + ': price not listed in the supplied 2024 menu PDF' : item.name + ': ₹' + price; return <div className={styles.menuItem} key={item.category + '-' + item.name}><span>{item.name}</span><i/><strong className={styles.menuPrice} style={{ animationDelay: Math.min(itemIndex, 12) * 35 + 'ms' }} title={price === null ? 'Price not listed in the supplied 2024 menu PDF' : '₹' + price} aria-label={label}>{price === null ? '—' : '₹' + price}</strong></div>; })}</div></article>; })}</div>}</div></section>
