...
const IMG = {
  ...
  thali: 'https://images.unsplash.com/photo-1601050690347-1bdc9a4fe1a8?auto=format&fit=crop&w=1400&q=88',
...
const pretty = (value: string) => FIX[value] ?? value;

function Arrow() { return <span aria-hidden="true">↗</span>; }
function SocialIcon({ size = 18 }: { size?: number }) {
...
function SafeImage({ src, alt, priority = false, sizes = '100vw', className = '' }: { src: string; alt: string; priority?: boolean; sizes?: string; className?: string }) {
  ...
  if (failed) return <div className={`${styles.fallback} ${className}`} role="img" aria-label={`${alt} unavailable`}><span>Raj Delight</span></div>;
...
export function RajDelightUltra() {
  ...
  const filtered = useMemo(() => {
    ...
    return allMenuItems.filter(item => (active === 'All' || item.category === active) && (!q || `${item.name} ${item.category}`.toLowerCase().includes(q)));
  }, [active, query]);
  ...
  const scrollTo = (id: string) => { document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); setMobile(false); };
  const choose = (name: string) => { setActive(name); setExpanded(name); scrollTo('menu'); };

  return (
    <main className={resolvedDark ? `${styles.site} ${styles.dark}` : styles.site}>
      <div className={styles.frame} aria-hidden="true" />
      <div className={styles.top}><div className={`${styles.wrap} ${styles.topin}`}><span>100% Vegetarian</span><i className={styles.dot}/><span>Chandausi</span><div className={styles.grow}/><a href={INSTAGRAM} target="_blank" rel="noreferrer"><SocialIcon size={14}/> @raj_delight</a><button onClick={() => setTheme(theme === 'light' ? 'dark' : theme === 'dark' ? 'system' : 'light')} aria-label="Cycle theme">{theme}</button></div></div>

      <header className={styles.nav}><div className={`${styles.wrap} ${styles.navin}`}><button className={styles.brand} onClick={() => scrollTo('home')} aria-label="Raj Delight home"><img className={styles.brandMark} src="/raj-delight-mark.svg" alt="Raj Delight"/><span className={styles.brandCopy}><b>Raj Delight</b><small>Chandausi</small></span></button><nav className={mobile ? `${styles.links} ${styles.linksOpen}` : styles.links}>{['about','menu','gallery','contact'].map(id => <button key={id} onClick={() => scrollTo(id)}>{id}</button>)}</nav><div className={styles.grow}/><div><a className={styles.navOrder} href={ZOMATO} target="_blank" rel="noreferrer">Order online <Arrow/></a><button className={styles.burger} onClick={() => setMobile(v => !v)} aria-label="Open navigation" aria-expanded={mobile}><span/><span/></button></div></div></header>

      <section className={styles.hero} id="home"><SafeImage src={IMG.biryani} alt="Vegetarian Indian biryani visual" priority sizes="100vw" className={styles.heroImg}/><div className={styles.heroShade}/><div className={`${styles.wrap} ${styles.heroIn}`}><div className={styles.heroCopy}><span className={styles.eyebrow}>Raj Delight · Chandausi</span><h1>Food that feels <em>like a celebration.</em></h1><p>North Indian favourites, South Indian classics, Indo-Chinese plates, pizzas, pasta, desserts and refreshing drinks.</p><div className={styles.heroActions}><button className={styles.goldBtn} onClick={() => scrollTo('menu')}>Explore menu <Arrow/></button><button className={styles.ghostBtn} onClick={() => scrollTo('gallery')}>View gallery <Arrow/></button></div><div className={styles.stats}><div><strong>4.3</strong><span>Google rating</span></div><div><strong>{menuCategories.length}</strong><span>menu categories</span></div><div><strong>9:30 AM–12 AM</strong><span>hours shown online</span></div><div><strong>Veg</strong><span>restaurant</span></div></div></div></div><div className={styles.scrollHint}>Scroll <span/></div></section>

      <section className={styles.orderBand}><div className={styles.wrap}><div className={styles.orderIn}><div><span className={styles.eyebrow}>Delivery · takeaway</span><h2>Your favourites, one click away.</h2></div><div className={styles.orderGrid}><a href={ZOMATO} target="_blank" rel="noreferrer" className={`${styles.orderCard} ${styles.zomato}`}><strong>Order on Zomato</strong><small>Live ordering</small><span><Arrow/></span></a><a href={SWIGGY} target="_blank" rel="noreferrer" className={`${styles.orderCard} ${styles.swiggy}`}><strong>Order on Swiggy</strong><small>Live ordering</small><span><Arrow/></span></a><a href={PHONE} className={styles.orderCard}><strong>Call restaurant</strong><small>+91 79831 48985</small><span><Arrow/></span></a></div></div></div></section>

      <section id="about" data-reveal="about" className={`${styles.section} ${styles.reveal} ${reveal.about ? styles.show : ''}`}><div className={styles.wrap}><div className={styles.story}><button className={styles.storyPhoto} onClick={() => setLightbox(0)}><SafeImage src={IMG.interior} alt="Raj Delight restaurant interior" sizes="(max-width:900px) 100vw, 55vw"/><span className={styles.caption}>Public listing photo · tap to enlarge</span></button><div className={styles.storyCopy}><span className={styles.eyebrow}>The Raj Delight experience</span><h2 className={styles.sectionTitle}>A little bit of <em>everything.</em></h2><p>Raj Delight is publicly listed in Chandausi with vegetarian dining, indoor seating, takeaway, lunch, dinner and home delivery.</p><div className={styles.features}><div><b>01</b><span>Vegetarian menu</span></div><div><b>02</b><span>Indoor seating</span></div><div><b>03</b><span>Takeaway & delivery</span></div><div><b>04</b><span>Lunch & dinner</span></div></div></div></div></div></section>

      <section data-reveal="specials" className={`${styles.section} ${styles.specials} ${styles.reveal} ${reveal.specials ? styles.show : ''}`}><div className={styles.wrap}><div className={styles.heading}><div><span className={styles.eyebrow}>Must-try plates</span><h2 className={styles.sectionTitle}>Favourites worth <em>coming back for.</em></h2></div><button className={styles.textLink} onClick={() => scrollTo('menu')}>View complete menu <Arrow/></button></div><div className={styles.specialGrid}>{specials.map((s,i)=><article className={styles.special} key={s.name}><button className={styles.specialPhoto} onClick={() => setLightbox(i+1)}><SafeImage src={s.img} alt={`${s.name} dish visual`} sizes="(max-width:760px) 84vw, 20vw"/><b className={styles.index}>{String(i+1).padStart(2,'0')}</b></button><small>{s.note}</small><h3>{s.name}</h3><strong className={styles.price}>{PRICE[s.name] ?? 'View live price'}</strong><button className={styles.view} onClick={() => choose(s.cat)}>View in menu <Arrow/></button></article>)}</div></div></section>

      <section className={styles.menu} id="menu"><div className={styles.wrap}><div className={styles.menuHead}><div><span className={styles.eyebrow}>The complete menu</span><h2 className={styles.menuTitle}>Every flavour. <em>One place.</em></h2><p>{menuCategories.length} categories · {allMenuItems.length} listed dishes. Search and browse category-by-category instead of scrolling through a wall of cards.</p></div><div className={styles.spend}><small>Public listing spend</small><b>₹200–₹400</b><span>average bill / person</span></div></div><div className={styles.menuTools}><label className={styles.search}><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search dishes or categories..." aria-label="Search dishes or categories"/>{query&&<button onClick={()=>setQuery('')} aria-label="Clear search">×</button>}</label><div className={styles.chips}>{['All',...topCategories].map(c=><button key={c} className={active===c?styles.activeChip:''} onClick={()=>{setActive(c);setExpanded(c==='All'?null:c)}}>{c==='All'?'All dishes':c}</button>)}</div><div className={styles.select}><label htmlFor="rd-cat">Browse category</label><select id="rd-cat" value={active} onChange={e=>{setActive(e.target.value);setExpanded(e.target.value==='All'?null:e.target.value)}}><option value="All">All dishes</option>{menuCategories.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}</select></div></div><div className={styles.meta}><span>{filtered.length} dishes shown</span>{active!=='All'&&<button onClick={()=>setActive('All')}>Reset</button>}</div><div>{menuCategories.map((c,i)=>{const items=filtered.filter(x=>x.category===c.name);if(active!=='All'&&!items.length)return null;const open=expanded===c.name||(query.trim().length>0&&items.length>0);return <section className={styles.cat} key={c.name}><button className={styles.catHead} onClick={()=>setExpanded(open?null:c.name)} aria-expanded={open}><span>{String(i+1).padStart(2,'0')}</span><strong>{c.name}</strong><small>{c.items.length} items</small><b>{open?'−':'+'}</b></button>{open&&<div className={styles.catItems}>{items.map(item=><a href={ZOMATO} target="_blank" rel="noreferrer" key={`${item.category}-${item.name}`}><span>{pretty(item.name)}</span><small>{PRICE[item.name] ?? PRICE[pretty(item.name)] ?? 'View live price'} <i>↗</i></small></a>)}</div>}</section>})}</div></div></section>

      <section className={styles.season}><button className={styles.seasonImg} onClick={()=>setLightbox(6)}><SafeImage src={IMG.thali} alt="Navratri Thali visual" sizes="(max-width:850px) 100vw, 55vw"/><span>Seasonal dish visual</span></button><div className={styles.seasonCopy}><span className={styles.eyebrow}>Seasonal selection</span><h2>Navratri <em>Thali.</em></h2><p>The public menu includes a dedicated Navratri Food section with a listed Navratri Thali.</p><button className={styles.darkBtn} onClick={()=>choose('Navratri Food')}>Explore Navratri food <Arrow/></button></div></section>

      <section className={styles.section} id="gallery"><div className={styles.wrap}><div className={styles.heading}><div><span className={styles.eyebrow}>Visual journal</span><h2 className={styles.sectionTitle}>See the place. <em>Feel the mood.</em></h2></div><p className={styles.headingText}>Tap any tile to open it full-screen. The restaurant photograph is identified separately from editorial dish visuals.</p></div><div className={styles.galleryGrid}>{gallery.map((g,i)=><button key={g.alt} className={`${styles.galleryTile} ${i===0?styles.tall:''}`} onClick={()=>setLightbox(i)}><SafeImage src={g.src} alt={g.alt} sizes="(max-width:700px) 50vw, 33vw"/><span>{g.label}</span><strong>{g.alt}</strong></button>)}</div></div></section>

      <section className={styles.contact} id="contact"><div className={styles.wrap}><div className={styles.contactGrid}><div><span className={styles.eyebrow}>Visit · order · connect</span><h2>Come for the food. <em>Stay for the moment.</em></h2><p>15, Lathi Bazar, Ward 05, Chandausi Locality, Chandausi, Uttar Pradesh 244412.</p><div className={styles.contactActions}><a href={MAPS} target="_blank" rel="noreferrer" className={styles.goldBtn}>Get directions <Arrow/></a><a href={PHONE} className={styles.outlineBtn}>Call restaurant</a></div></div><div className={styles.contactCard}><div><small>Hours shown online</small><b>9:30 AM – 12:00 AM</b></div><div><small>Phone</small><b>+91 79831 48985</b></div><div><small>Food licence</small><b>12726074000015</b></div><a href={INSTAGRAM} target="_blank" rel="noreferrer" className={styles.instagram}><SocialIcon size={28}/><span>@raj_delight</span><b><Arrow/></b></a></div></div></div></section>
      ...
      <div className={styles.desktopOrder}><a href={ZOMATO} target="_blank" rel="noreferrer">Order online <Arrow/></a></div><nav className={styles.mobileActions}><button onClick={()=>scrollTo('menu')}>Menu</button><a href={ZOMATO} target="_blank" rel="noreferrer">Order</a><a href={PHONE}>Call</a><a href={MAPS} target="_blank" rel="noreferrer">Directions</a></nav>
      {lightbox !== null && <div className={styles.lightbox} role="dialog" aria-modal="true" aria-label="Image viewer" onClick={()=>setLightbox(null)}><button className={styles.close} onClick={()=>setLightbox(null)} aria-label="Close">×</button><button className={styles.prev} onClick={e=>{e.stopPropagation();setLightbox((lightbox-1+gallery.length)%gallery.length)}} aria-label="Previous image">‹</button><div className={styles.lightInner} onClick={e=>e.stopPropagation()}><SafeImage src={gallery[lightbox].src} alt={gallery[lightbox].alt} priority sizes="94vw"/><p>{gallery[lightbox].alt}</p></div><button className={styles.next} onClick={e=>{e.stopPropagation();setLightbox((lightbox+1)%gallery.length)}} aria-label="Next image">›</button></div>}
    </main>
  );
}
