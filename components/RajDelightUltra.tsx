...
export function RajDelightUltra() {
  ...
  useEffect(() => {
    ...
    if (!('IntersectionObserver' in window)) { els.forEach(el => el.classList.add(styles.show)); return; }
  ...
  const choose = (category: string) => { setActive(category); setExpanded(category); go('menu'); };

  return (
    <main className={isDark ? `${styles.site} ${styles.dark}` : styles.site}>
      ...
      <section className={styles.hero} id="home"><SafeImage src={IMG.biryani} alt="Vegetarian biryani dish visual" priority sizes="100vw" className={styles.heroImg}/><div className={styles.heroShade}/><div className={`${styles.wrap} ${styles.heroIn}`}><span className={styles.eyebrow}>Raj Delight · Chandausi</span><h1>Food that feels <em>like a celebration.</em></h1><p>North Indian favourites, South Indian classics, Indo-Chinese plates, pizzas, pasta, desserts and refreshing drinks.</p><div className={styles.heroActions}><button className={styles.goldBtn} onClick={()=>go('menu')}>Explore menu <Arrow/></button><button className={styles.ghostBtn} onClick={()=>go('gallery')}>View gallery <Arrow/></button></div><div className={styles.stats}><div><strong>4.3</strong><span>Google rating</span></div><div><strong>{menuCategories.length}</strong><span>menu categories</span></div><div><strong>9:30 AM–12 AM</strong><span>hours shown online</span></div><div><strong>Veg</strong><span>restaurant</span></div></div></div><div className={styles.scrollHint}>Scroll <span/></div></section>
      ...
      <section className={styles.section} id="gallery"><div className={styles.wrap}><div className={styles.heading}><div><span className={styles.eyebrow}>Visual journal</span><h2 className={styles.sectionTitle}>See the place. <em>Feel the mood.</em></h2></div><p className={styles.headingText}>Tap any tile to view it full-screen. The restaurant photograph is identified separately from editorial dish visuals.</p></div><div className={styles.galleryGrid}>{gallery.map((g,i)=><button key={g.alt} className={`${styles.galleryTile} ${i===0?styles.tall:''}`} onClick={()=>setLightbox(i)}><SafeImage src={g.src} alt={g.alt} sizes="(max-width:700px) 50vw, 33vw"/><span>{g.label}</span><strong>{g.alt}</strong></button>)}</div></div></section>
